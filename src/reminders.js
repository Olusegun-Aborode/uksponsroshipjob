...
async function runReminders() {
  const due = dueSoon(), follow = needFollowup();
  if (!due.length && !follow.length) return { due: 0, followup: 0, sent: false };
  let msg = '';
  if (due.length) msg += `⏰ Deadlines within ${DEADLINE_DAYS} days\n` + due.slice(0, 10).map(j => `• ${j.title} — ${j.employer} (in ${j.d}d, ${j.deadline})`).join('\n') + '\n\n';
  ...
  return { due: due.length, followup: follow.length, sent };
}
...
module.exports = { runReminders, dueSoon, needFollowup };
